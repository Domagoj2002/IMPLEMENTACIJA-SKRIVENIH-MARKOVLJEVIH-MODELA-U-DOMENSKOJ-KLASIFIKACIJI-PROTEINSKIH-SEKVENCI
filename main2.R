...
# ... [ostaje isto do definicije results data.frame]
...
for (fasta_file in fasta_files) {
  ...
  for (i in seq_along(seqs)) {
    ...
    # Ako ima nepoznatih simbola za hmm, preskoči
    if (!all(aa_seq %in% symbols_global)) {
      cat("⚠️ Sekvenca sadrži nepoznate simbole – preskačem\n")
      next
    }
    ...
    for (domain in all_domains) {
      ...
      log_p_hmm <- calc_log_prob(hmm, aa_seq, TRUE)
      log_p_bg <- calc_log_prob(hmm_bg, aa_seq, FALSE)
